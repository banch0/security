...
func printUsage() {
	...
	Usage:
	` + os.Args[0] + ` <privateKeyFilename> <messageFilename>
	<signatureFilename>
	Example:
...
func checkArgs() (string, string, string) {
	// Need exactly 3 argument provided
	if len(os.Args) != 4 {
		printUsage()
		os.Exit(1)
	}
...
// Load the RSA private key from PEM encoding file
func loadPivateKeyFromPemFile(privateKeyFilename string) *rsa.PrivateKey {
...
func main() {
	...
	privateKey := loadPivateKeyFromPemFile(privateKeyFilename)
