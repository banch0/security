...
func main() {
	// Load parameters
	privateKeyFilename, crsOutFilename := checkArgs()
	privKey := loadPrivateKeyFromPemFile(privateKeyFilename)
	...
	// Prepare CRS template\
	crsTemplate := x509.CertificateRequest{
		...
		DNSNames:       []string{"Bussines Unit Name"},
		EmailAddresses: []string{"test@localhost"},
		IPAddresses:    []net.IP{},
	}

	// Create a CRS based of the template
	crs, err := x509.CreateCertificateRequest(rand.Reader, &crsTemplate, privKey)
	if err != nil {
		log.Fatal("Error creating certificate signing request. ", err)
	}

	saveCSRToPemFile(crs, crsOutFilename)
}
